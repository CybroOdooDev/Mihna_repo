...
class UserPortal(CustomerPortal):
    ...
    def _prepare_portal_counter_values(self, counter):
        """Prepare portal counter values for verification requests assigned to
        the logged-in partner agency."""
        if counter == 'verification_count':
            partner = request.env.user.partner_id
            return 'employee.verification', [('state', '=', 'assign'), ('agency_id', '=', partner.id)], 'sudo'
