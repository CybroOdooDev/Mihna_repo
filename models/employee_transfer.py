...
class EmployeeTransfer(models.Model):
    ...
    responsible_employee_id = fields.Many2one(
        ...
        default=lambda self: self.env['hr.employee'].sudo().with_context(active_test=False).search([('user_id', '=', self.env.uid)], limit=1),
        readonly=True,
        help="The person responsible for the transfer.")
    new_department_id = fields.Many2one('hr.department', string='Target Department', help="The department the employee will be transferred to.")
    new_job_id = fields.Many2one('hr.job', string='Target Job Position', help="The job position the employee will hold after transfer.")
    new_work_location_id = fields.Many2one('hr.work.location', string='Target Work Location', help="The work location after transfer.")
