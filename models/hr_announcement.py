...
class HrAnnouncement(models.Model):
    ...
    acknowledged_employee_ids = fields.Many2many(
        ...
        string='Acknowledged Employees', copy=False)
    ...
    def _compute_has_acknowledged(self):
        for record in self:
            if self.env.user.employee_id in record.sudo().acknowledged_employee_ids:
    ...
    def get_active_announcements(self):
        ...
        for a in announcements:
            ...
            if is_targeted:
                if not user_emp_id or user_emp_id not in a.sudo().acknowledged_employee_ids.ids:
                    result.append({
                        'id': a.id,
