...
class HrResignation(models.Model):
        ...
        for resignation in self:
            ...
            if open_custodies:
                ...
                if admin_lines:
                    for line in admin_lines:
                        ...
                        if line.responsible_user_id:
                            resignation.activity_schedule(
                                ...
                                summary=_('Clearance Check: Custody Recovery'),
                                note=_('Please review the unreturned custody checklist for %s.') % resignation.employee_id.name
                            )
                else:
                    ...
                    custody_line = self.env['hr.resignation.clearance.line'].sudo().create({
                        'resignation_id': resignation.id,
                        'clearance_type_id': c_type.id,
                        'responsible_user_id': c_type.default_responsible_id.id or self.env.uid,
                        'state': 'blocked',
                    ...
                    if custody_line.responsible_user_id:
                        resignation.activity_schedule(
                            'mail.mail_activity_data_todo',
                            user_id=custody_line.responsible_user_id.id,
                            summary=_('Clearance Check: Custody Recovery'),
                            note=_('Please review the unreturned custody checklist for %s.') % resignation.employee_id.name
                        )
    ...
    def action_compute_settlement(self):
        ...
        for resignation in self:
            ...
            # Fetch Salary Advance (Approved or Paid advances with open balance)
            if 'salary.advance' in self.env and resignation.employee_id:
                ...
                advances = self.env['salary.advance'].search([
                    ('employee_id', '=', resignation.employee_id.id),
                    ('state', 'in', ['approved', 'paid', 'approve']),
                ])
                if 'balance_amount' in self.env['salary.advance']._fields:
                    advance_recovery = sum(a.balance_amount for a in advances if a.balance_amount > 0)
                else:
                    advance_recovery = sum(a.advance for a in advances if
                                           a.date and a.date.month == target_date.month and a.date.year == target_date.year)
