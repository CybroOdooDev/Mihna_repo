...
class ResPartner(models.Model):
    ...
    def _l10n_om_flick_get_party_payload(self):
        ...
        return {
            ...
            # Not a geographic state/governorate code (Odoo's res.country.state has no such concept
            # for Oman) - Flick's live validator only accepts one of Oman's 4 named free zones
            # (SHRFZ/SEZAD/SLLFZ/AFZ) or "MO" for Mainland Oman. Odoo has no field recording
            # free-zone registration, so default every partner to "MO" - true for the vast majority
            # of businesses.
