...
class SalaryAdvance(models.Model):
    ...
    def action_submit(self):
        ...
        for rec in self:
            ...
            min_days = rec.company_id.salary_advance_min_days
            if min_days > 0:
                start_date = rec.employee_id.create_date.date()
                if start_date:
                    employment_days = (rec.date - start_date).days
                    if employment_days < min_days:
                        raise exceptions.UserError(_(
                            'This employee has only been employed for %s days. A minimum of %s days is required to request a salary advance.'
                        ) % (employment_days, min_days))
