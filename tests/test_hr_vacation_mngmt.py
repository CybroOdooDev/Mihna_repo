...
class TestHrVacationMngmt(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.employee_user = cls.env['res.users'].with_context(
            ...
            'name': 'Employee User',
            'login': 'employee_user_vacation@test.com',
        })

        cls.employee = cls.employee_user.employee_id or cls.env['hr.employee'].search([('user_id', '=', cls.employee_user.id)], limit=1)
        if cls.employee:
            cls.employee.write({
                'name': 'Alice Engineer',
        ...
        cls.leave_type = cls.env['hr.work.entry.type'].search([('requires_allocation', '=', False)], limit=1)
    ...
    def test_action_confirm_ticket_raises_when_no_expense_account_config(self):
        ...
        self.env['ir.config_parameter'].sudo().set_int(
            'hr_vacation_mngmt.expense_id', 0
        )
        leave = self._make_leave()
    ...
    def test_task_reassign_raises_when_assignee_is_unavailable(self):
        ...
        """
        self._make_leave(employee=self.employee2, state='validate')
        leave = self._make_leave(state='confirm')
    ...
    def test_res_config_settings_leave_reminder_field(self):
        ...
        settings.execute()
        param = self.env['ir.config_parameter'].sudo().get_bool(
